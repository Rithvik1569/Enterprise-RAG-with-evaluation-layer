...
import AuthPage, { type UserInfo } from './components/AuthPage';
...
export default function App() {
  ...
  // View state: chat or admin analytics
  ...
  }, [chatHistory, chatLoading, currentView]);
  ...
  const fetchAnalytics = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error('Failed to load admin analytics reports');
      }
      const data = await res.json();
  ...
  // Handlers
  // ------------------------------------------------------------------
  const handleAuthSuccess = useCallback((newToken: string, userInfo: UserInfo) => {
    setToken(newToken);
    setUser(userInfo);
  }, []);
  ...
  const handleUpload = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        const errData = await res.json().catch(() => ({ detail: 'Failed to process document' }));
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    const newHistory: ChatMessage[] = [...chatHistory, { sender: 'user', text: userMsgText }];
  ...
  const drawSparkline = (dataList: any[], key: string, width: number = 320, height: number = 120) => {
                    ...
                    <div className="overflow-x-auto border border-slate-900 rounded-xl">
                      <table className="w-full text-left text-xs border-collapse">
                        ...
                        <tbody className="divide-y divide-slate-900/60">
                          {analyticsData.recent_evaluations.map((ev: any) => (
